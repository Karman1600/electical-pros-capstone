...
export default function Component() {
  ...
  const originalAppliances = JSON.parse(appliancesJSON) || null;
  const appliances = originalAppliances.map((appliance) => ({
    ...appliance,
    spent: 0,
    status: "In Progress",
  })) || null;
  ...
  const appliancesTotal =
    isAdvancedPlan && appliances
      ? appliances.reduce((sum, appliance) => sum + appliance.price, 0)
      : 0;
